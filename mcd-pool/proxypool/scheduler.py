...
class Scheduler():
    ...
    def run_tester(self, cycle=CYCLE_TESTER):
        ...
        if not ENABLE_TESTER:
            logger.info('tester未启动, 退出')
            return
        tester = Tester()
        loop = 0
        while True:
            logger.debug(f'tester 循环 {loop} 启动...')
            tester.run()
            loop += 1
            time.sleep(cycle)
    ...
    def run_getter(self, cycle=CYCLE_GETTER):
        ...
        if not ENABLE_GETTER:
            logger.info('getter未启动, 退出')
            return
        getter = Getter()
        loop = 0
        while True:
            logger.debug(f'getter 循环 {loop} 启动...')
            getter.run()
            loop += 1
            time.sleep(cycle)
    ...
    def run_server(self):
        ...
        if IS_PROD:
            if APP_PROD_METHOD == APP_PROD_METHOD_GEVENT:
                ...
                logger.error("不受支持的 APP_PROD_METHOD")
                return
        else:
    ...
    def run(self):
        ...
        try:
            ...
            if ENABLE_TESTER:
                ...
                logger.info(f'正在启动tester, 进程pid {tester_process.pid}...')
                tester_process.start()

            if ENABLE_GETTER:
                getter_process = multiprocessing.Process(
                    target=self.run_getter)
                logger.info(f'正在启动getter, 进程pid {getter_process.pid}...')
                getter_process.start()

            if ENABLE_SERVER:
                server_process = multiprocessing.Process(
                    target=self.run_server)
                logger.info(f'正在启动server, 进程pid {server_process.pid}...')
