...
class RedisClient(object):
    ...
    def __init__(self, host=REDIS_HOST, port=REDIS_PORT, password=REDIS_PASSWORD, db=REDIS_DB,
        ...
        """
        init redis client初始化
        :param host: redis host 192.168.137.160
        :param port: redis port 6379
        :param password: redis password
        :param connection_string: redis connection_string
        """
        # if set connection_string, just use it；如果设置了连接字符串，直接使用它来创建 Redis 客户端
    ...
    def random(self) -> Proxy:
        """
        得到随机ip，尝试写入和写入最高分。如果不存在，按照分数排序或报错
        get random proxy
    ...
    def decrease(self, proxy: Proxy) -> int:
        ...
        logger.info(f'{proxy.string()} 分数-1, 当前分数 {score}')
        if score <= PROXY_SCORE_MIN:
            logger.info(f'{proxy.string()} 当前分数 {score}, 删除')
    ...
    def max(self, proxy: Proxy) -> int:
        ...
        :return: new score
        """
        logger.info(f'{proxy.string()} 有效, 设置分数为 {PROXY_SCORE_MAX}')
        if IS_REDIS_VERSION_2:
