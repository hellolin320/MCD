...
def spark_read_csv():
    ...
    schema = StructType([
        ...
        StructField("starScore", StringType(), True),
        StructField("tasteScore", StringType(), True),
        StructField("envScore", StringType(), True),
        StructField("serScore", StringType(), True),
        StructField("goodCount", StringType(), True),
        StructField("middleCount", StringType(), True),
        StructField("badCount", StringType(), True)
    ])
...
if __name__ == '__main__':
    ...
    # 如果 starScore 为空，则 starScore=（tasteScore+envScore+serScore）/3
    filled_spark_df = cleaned_spark_df_2.withColumn(
        "starScore",
        when(col("starScore") == '', (col("tasteScore") + col("envScore") + col("serScore")) / 3).otherwise(
            col("starScore"))
    )

    # 数据处理：填充 starScore 为空的值
    cleaned_spark_df_3 = filled_spark_df
