...
"""
author:Lin
读取 HDFS 上的文件，将其转换为 Pandas DataFrame，然后再将其转换为 Spark DataFrame
将 Pandas DataFrame 的 NaN 值转换为” “，然后删除空白值和重复值
"""
import glob
import sys
import pandas as pd
...
def spark_read_csv():
    ...
    schema = StructType([
        ...
        StructField("recommendFood", StringType(), True),
        StructField("userCommentTime", StringType(), True),
        StructField("userScore", StringType(), True),
        StructField("userComment", StringType(), True),
    ])
...
if __name__ == '__main__':
    ...
    # 将评论为空的行删除
    cleaned_spark_df_1 = spark_df.filter(col("userComment") != '')
    # 清洗后 PySpark DataFrame 的行数
    cleaned_row_count_1 = cleaned_spark_df_1.count()
    print("删除空白值后的行数：", cleaned_row_count_1)

    # 将评论重复的行删除
    cleaned_spark_df_2 = cleaned_spark_df_1.dropDuplicates(['userComment'])
    ...
    save_to_hdfs(cleaned_spark_df_2, hdfs_output_path)
