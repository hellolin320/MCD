...
def detail_page(detail_url,headers,proxies):
    ...
    write_to_csv(os.path.join(base_dir, '/data/raw/good.csv', fieldnames, comment_info))
    return comment_info
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(os.path.join(base_dir, 'data/raw/good.csv')):
        with open(os.path.join(base_dir, 'data/raw/good.csv'), mode='a', encoding='utf-8', newline='') as f:
            csv_writer = csv.DictWriter(f, fieldnames=fieldnames)
    ...
    for shop in list:
        ...
        shop_url=f"{shop}/review_all?queryType=reviewGrade&queryVal=good"
        p=int(get_page(shop_url,headers))+1
        print("最大页数=",p-1)
        for i in range(current_page,p):
            sleep_time=random.uniform(40,50)
            time.sleep(sleep_time)
            herf=f"{shop_url}/p{i}?queryType=reviewGrade&queryVal=good"
